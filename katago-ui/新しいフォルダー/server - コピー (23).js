...
const readline = require("readline");
 ...
 // 起動時に確認ログ
 logEngine('easy', ENGINES.easy);
 logEngine('normal', ENGINES.normal);
 logEngine('hard', ENGINES.hard);
 ...
 rlErr.on('line', (line) => {
  ...
  try {
    ...
    if (m) {
     ...
     // ついでに KataGo 自体のバージョンも拾う（例: "KataGo v1.16.3"）
     m = msg.match(/KataGo v(\d+\.\d+\.\d+)/i);
     if (m) {
       engineMeta[name] = engineMeta[name] || {};
       engineMeta[name].katago = m[1];
     }
    } catch {}
 });
 // プロセス終了時に Readline を閉じてリーク防止
