...
// Render本番は /app 固定。ローカル実行時はカレントの疑似/appを使う
const ROOT = process.env.RENDER ? '/app' : path.join(process.cwd(), 'app');
...
sh(`
  ...
  tar xzf katago.tgz
  rm -f katago.tgz
  # 展開された中の実行ファイルを katago に揃える
  find . -type f -name "katago" -print -exec mv {} ./katago \\; || true
  chmod +x ./katago
`);

// 2) 軽量 b6 ウェイトをダウンロード（まずは動作確認用）
sh(`
  set -e
  ...
  # normal/hard も当面は同じファイルでOK（あとで b10/b18 に差し替え推奨）
